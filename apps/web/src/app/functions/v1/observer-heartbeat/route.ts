import { handleHeartbeat, failure } from "@observer/sources";

import { observerDeps } from "@/lib/sources/deps";

/**
 * `POST /functions/v1/observer-heartbeat`
 *
 * Records a source's operational state. Never creates an analytics fact.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  return handleHeartbeat(request, deps);
}
