...
- **Hierarchy** (`tokens.css`, the base scale underlying `obs-`/legacy `iris-`; `ox-`/`ask-` build their own component-level sizes on top of the same family):
  ...
  **This `tokens.css` table is the base scale inherited by `obs-`/legacy `iris-`. It is not what `ask-`/`ox-` actually render at the component level.** Extracted directly from the live stylesheets rather than assumed:
  ...
  | Section/field kicker | `--ox-micro` 0.75rem (12px), `--weight-semibold`, uppercase, tracking `--ox-track-micro` (0.09em) — `.ox-kicker`, `.ox-context-label`, `.ox-subhead`, `.ox-field-label`, `.ox-tally-item dt`, sticky table headers, table `data-label` (7 rules, one consistent treatment) | `.ask-panel-title` 13px / medium / tracking 0.22em, "styled as an uppercase eyebrow label" — the same job (labels a section or panel) at `ask-`'s own considered size, not `ox-`'s | *(the 7px `.irs-brand-sub` badge is retired into this role — see decision below)* | `--ox-micro` canonical for `ox-`/shell contexts; `ask-` keeps its own 13px/0.22em by design | **RESOLVED, not drift**: this is one role (labels a section, field, or table column), and it was already one treatment in 7 places — the table previously compared it against a different role by mistake. See the decision note below the table. |
  | Inline meta value | `--ox-micro` 0.75rem (12px), regular weight, no transform, normal tracking, `--ox-ink-3`/`--ox-ink-4` — `.ox-n`, `.ox-tab-count`, `.ox-time-when`, `.ox-chart-period`, `.ox-thread-when`, `.ox-person-role`, `.ox-stack-label`, `.ox-answer-figure dt` (10 rules, one consistent treatment) | *(not named separately in the original table; no `ask-` drift identified for this role)* | *(none identified)* | `--ox-micro`, plain | **RESOLVED, not drift**: a genuinely different job from the kicker above — displays a small piece of data (a timestamp, a count, a role, a sample size) inline, and reads as data specifically because it is *not* uppercase or tracked. Same size token as the kicker, deliberately un-emphasized otherwise. |
