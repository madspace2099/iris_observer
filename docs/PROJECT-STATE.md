...
| **M10 CRM connectors**                       | 🟡 **built and read by the product** (`7226e07` + the catalogue seam, ADR-0036) — adapters, sync, persistence, screen, Project drawing a delivered stock, sparse units drawn with their gaps said in words, deals delivered, diffed into stage facts, stored, shown on the integrations screen and drawn on Sales Flow's ladder. Open: the REALPAD deals adapter (Excel, needs one real export) and the Preview prerequisites in the next-action list |
