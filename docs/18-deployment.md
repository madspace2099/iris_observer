...
**client-exposed**: Next.js inlines a variable into the browser bundle if and only if its name begins
with `NEXT_PUBLIC_`, so `SUPABASE_URL` is **not** browser-bundled. And client exposure does not
establish the server target — what the browser was built to call and what the route handler writes to
...
set-and-usable wins. An **absent** `SUPABASE_URL` therefore does not mean "no destination" — the
server silently falls back to the browser-exposed variable. Record that as a **fallback in effect**,
not as a mapping, and treat it as a finding. (A **malformed** `SUPABASE_URL` does not fall back at
all: the resolver stops at the first name that is set and reports it unusable.)
